...
func (p *Person) Clone() Prototype {
	newP := *p
	newHobby := make([]string, len(p.HObby))

	copy(newHobby, p.HObby)
...
// arjunsingh@surajsingh 04 PrototypePattern % go run main.go
// Orignal : {ArjunSingh 28  [Coding Badminton]}
// Clone : &{Kabbu 4  [dancing Badminton]}
// arjunsingh@surajsingh 04 PrototypePattern %
